...
/*  This module processes Cosmwasm CW20 transfers at Cosmos SDK blockchains.
...
abstract class CosmosCW20Module extends CoreModule
{
    use CosmosTraits;
    ...
    public ?array $extra_data_details = [
        CosmosSpecialTransactions::Mint->value  => 'Mint',
        CosmosSpecialTransactions::Burn->value  => 'Burn',
    ];
    ...
    final public function pre_process_block($block_id)
    {
        ...
        for ($i = 0; $i < $tx_count; $i++)
        {
            ...
            foreach ($tx_result['events'] as $tx_event)
            {
                ...
                {
                    case 'wasm':
                        $info = $this->parse_wasm_cw20_event($tx_event['attributes']);
                        if (is_null($info))
                            break;
        ...
        {
            $request = base64_encode('{"token_info":{}}');
