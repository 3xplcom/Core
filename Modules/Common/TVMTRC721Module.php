...
/*  This module works with the TRC-721 NFT (requires vm.supportConstant) standard, see
 *  https://github.com/tronprotocol/tips/blob/master/tip-721.md */

abstract class TVMTRC721Module extends CoreModule
{
    use TVMTraits;
        ...
        foreach ($receipt_data as $receipt)
        {
            ...
            foreach ($receipt['log'] as $log)
            {
                if (!isset($log['topics'])) // this also happens block 34998371
                    continue;
                if (count($log['topics']) !== 4 || $log['topics'][0] != 'ddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef')
                ...
                $events[] = [
                    ...
                    'sort_key' => $sort_key++,
                    'effect' => '-1',
                    'extra' => to_int256_from_0xhex('0x' . $log['topics'][3])
                ];
    ...
    function api_get_balance(string $address, array $currencies): array
    {
        ...
        // Input currencies should be in format like this: `tvm-trc-20/TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t`
