...
/*  This module processes transfers of all Stellar assets. Requires a Stellar Horizon API  */
...
abstract class StellarLikeMainModule extends CoreModule
{
    ...
    public ?array $special_addresses = ['the-void', 'liquidity-pool', 'sdex', 'operations'];
    ...
    public ?array $transaction_fees = null;
    ...
    final public function pre_process_block($block_id)
    {
        ...
        $last_transaction = $operations[0]['transaction_hash'];
        foreach ($operations as $op) 
        {
            $tx_is_failed = $op['transaction_successful'] === true ? false : true; // yes, it's success but for is_failed it will be correct
            if($last_transaction != $op['transaction_hash']) {
                $events[] = [
                    'transaction' => $last_transaction,
                    'currency' => null,
                    'address' => $this->transaction_fees[$last_transaction]['fee_account'],
                    'sort_key' => $sort_key++,
                    'effect' => '-' . $this->transaction_fees[$last_transaction]['fee_charged'],
                    'failed' => false,
                    'extra' => 'f',
                ];
                ...
                $events[] = [
                    'transaction' => $last_transaction,
                    'currency' => null,
                    'address' => $this->transaction_fees[$last_transaction]['source_account'],
                    'sort_key' => $sort_key++,
                    'effect' => '-0',
                    'failed' => false,
                ...
                $last_transaction = $op['transaction_hash'];
            }
            switch ($op['type']) {
